...
`lean_lake_cache` also takes `sub_dir` (the Lake project's subdirectory in the repo, when its
`lakefile`/`lean-toolchain` is not at the root) and `build_targets` (Lake libraries to build *from
source* after the cache is fetched — their deps come prebuilt, their oleans are included too).
Together these let you ingest a source library that sits on top of a cached mathlib.
...
This is an early (`0.2.0`) ruleset. Known limitations:
...
- **macOS is best-effort.** Linux (x86_64, aarch64) gates CI; the macOS integration job runs the full
  toolchain suite too, but non-gating (the `cc_deps` / reverse-FFI paths depend on the runner's clang
  + SDK resolving).
...
- **No from-source build of mathlib-scale libraries.** Ingest their prebuilt `.olean` cache instead —
  `lean_lake_cache` (runs `lake exe cache get`) or `lean_olean_archive` (a tarball you host). A real
  mathlib cache is exercised by [`examples/mathlib`](examples/mathlib) as a non-gating CI job (the
  cache is several GB); no binary `.olean` fixture is committed (it would pin one toolchain version).
...
examples/             # mathlib prebuilt-olean example (heavy, separate module)
